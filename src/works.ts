...
function InitEnv(): boolean {
    for (const exportCfg of gCfg.Export as utils.ExportCfg[]) {
        const Constructor = utils.ExportWrapperMap.get(exportCfg.type);
        if (Constructor === undefined) {
            utils.exceptionRecord(
                utils.red(
                    `Export is not currently supported for the current type "${utils.yellow_ul(
                        exportCfg.type
...
export async function execute(): Promise<boolean> {
    ...
    if (!excel_utils.HandleDataTableStep2()) {
...
async function HandleExcelFileWork(fileName: string, cb: (ret: boolean) => void): Promise<void> {
    ...
    try {
        ...
        if (ext === '.csv') ret = await excel_utils.HandleCsvFile(fileName);
            ...
            ret = await excel_utils.HandleExcelFile(fileName);
        else ret = true;
    } catch (ex) {
        utils.logger(`handle fileName failure. ${String(ex)}`);
    }
    cb(ret);
...
async function HandleDir(dirName: string, cb: (ret: boolean) => void): Promise<void> {
    ...
    pa.forEach(function (fileName) {
        ...
        if (!info.isFile())
            HandleDir(filePath, cb).then(utils.EmptyCallback).catch(utils.EmptyCallback);
        else HandleExcelFileWork(filePath, cb).then(utils.EmptyCallback).catch(utils.EmptyCallback);
    });
    WorkerMonitor.decWork();
}
...
async function HandleReadHeader(): Promise<boolean> {
    ...
    for (let fileOrPath of gCfg.IncludeFilesAndPath) {
        ...
        if (fs.statSync(fileOrPath).isDirectory()) {
            HandleDir(fileOrPath, cb).then(utils.EmptyCallback).catch(utils.EmptyCallback);
        } else if (fs.statSync(fileOrPath).isFile()) {
            HandleExcelFileWork(fileOrPath, cb)
                .then(utils.EmptyCallback)
                .catch(utils.EmptyCallback);
        } else {
    ...
    utils.logger(
        `${utils.green('[SUCCESS]')} READ ALL SHEET DONE. Total Use Tick : ${utils.green(
