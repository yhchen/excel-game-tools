...
type TNode = { [key: string]: unknown };
type TRootNode = TNode & { ids: Array<unknown> };
...
function exportToSingleLuaContent(
    exportWrapper: utils.IExportWrapper,
    sheetName: string,
    header: Array<utils.SheetHeader>,
    jsObj: TRootNode,
    shortName = false
): IExportToSingleLuaData {
    ...
    for (const id of jsObj['ids']) {
        const objLst = new Array<string>();
        const jsObjSingle = jsObj[id as string] as TNode;
        for (const hdr of header) {
            const name = exportWrapper.TranslateColName(hdr.name);
            if (jsObjSingle[name] === undefined) continue;
            objLst.push(
                `\t\t[${hdr.shortName}] = ${json_to_lua.jsObjectToLua(jsObjSingle[name])},`
            );
        }
        tableLst.push(
            `\t${json_to_lua.makeLuaKey(<string>id)} = {${LF}${objLst.join(LF)}${LF}\t},`
        );
    }
...
class LuaExport extends utils.IExportWrapper {
    ...
    protected async ExportTo(dt: utils.SheetDataTable): Promise<boolean> {
        ...
        try {
            const dataCtx = exportToSingleLuaContent(
                this,
                dt.sheetName,
                arrExportHeader,
                jsonObj,
                this._exportCfg.UseShortName
            );
    ...
    protected async ExportGlobal(): Promise<boolean> {
        ...
        const luacontent = FMT.replace(
            '{data}',
            json_to_lua.jsObjectToLuaPretty(this._globalObj, 3)
        );
