		...
		for (const key in kv) {
			...
			if (val.__inner__o_type__abcxyz__ == 'array') {
				if ((val.__inner__type__abcxyz__)?.__inner__o_type__abcxyz__ == 'array') {
					typeName = this.GenArrayMessageAndGetTypeName(val.__inner__type__abcxyz__, gCfg.LineBreak);
				} else {
...
Array(Enum({
	...
	private async ExportData(dt: utils.SheetDataTable, outputFile: string) {
		...
		if (arrTypeHeader == undefined || arrTypeHeader.length <= 0) {
			console.info(`Pass Sheet ${utils.yellow_ul(dt.sheetName)}: ignore export data ${outputFile}`);
			return;
		}
