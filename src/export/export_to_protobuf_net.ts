...
class ProtobufNetExport extends utils.IExportWrapper {
    ...
    private readonly ARR_FILE_HEADER =
        '/** This is an automatically generated class by excel export tools. Please do not modify it. **/' +
        gCfg.LineBreak +
        gCfg.LineBreak +
        gCfg.LineBreak;
    ...
    protected async ExportTo(dt: utils.SheetDataTable): Promise<boolean> {
        ...
        if (FMT.indexOf('{classDef}') < 0) {
            utils.exception(
                `[Config Error] ${utils.yellow_ul(
                    'Export.ExportTemple'
                )} not found Keyword ${utils.yellow_ul('{classDef}')}!`
            );
        }
        ...
        if (this._exportCfg.UseNamespace) {
            for (const using of this._exportCfg.UseNamespace) {
                usingContext += `using ${using};${gCfg.LineBreak}`;
            }
            usingContext += gCfg.LineBreak;
        }
        // generate table proto
        const namespaceContext = this._exportCfg.Namespace
            ? `namespace ${this._exportCfg.Namespace}${gCfg.LineBreak}{${gCfg.LineBreak}`
            : undefined;
        const name = dt.sheetName;
        const sheetType = this.GenSheetType(name, dt, FMT);
        ...
        for (const [name, define] of this._leadTypeMap) {
            ...
            // ignore type...
            if (
                this._exportCfg.IgnoreTypeExport !== undefined &&
                this._exportCfg.IgnoreTypeExport.indexOf(name) >= 0
            ) {
            ...
            if (this._exportCfg.Namespace) {
                leadTypeMessage +=
                    `namespace ${this._exportCfg.Namespace}${gCfg.LineBreak}` +
                    `{${gCfg.LineBreak}` +
                    `${define.split(gCfg.LineBreak).join(`${gCfg.LineBreak}    `)}${
                        gCfg.LineBreak
                    }` +
                    `}${gCfg.LineBreak}`;
            } else {
    ...
    protected async ExportGlobal(): Promise<boolean> {
        ...
        for (const dataTable of array) {
            const name = dataTable.sheetName;
            const sheetType = this.GenSheetType(name, dataTable, FMT);
            if (sheetType) {
                exTypeData += `${sheetType}${gCfg.LineBreak}${gCfg.LineBreak}`;
            }
        }

        const NamespaceContent = this._exportCfg.Namespace
            ? `namespace ${this._exportCfg.Namespace}${gCfg.LineBreak}{${gCfg.LineBreak}`
            : undefined;
        let leadTypeMessage = '';
        for (const [, value] of this._leadTypeMap) {
        ...
        data +=
            `${NamespaceContent ?? ''}${gCfg.LineBreak}` +
            exTypeData +
    ...
    private GenSheetType(
        ...
        for (const header of arrExportHeader) {
            ...
            if (
                !isBaseType &&
                !isNotNull &&
                this._exportCfg.NullableReferenceTypes &&
                !header.parser.IsStructMode
            ) {
                data +=
                    gCfg.LineBreak +
                    comment +
                    ...
                    `    public ${typeName}? ${this.TranslateColName(header.name)} ${GET_AND_SET}${
                        gCfg.LineBreak
                    }` +
        ...
        const arrTypeContext =
            ...
            `    public ${ARRAY_TYPE(sheetName)} ${this.TABLE_ROW_NAME} ${GET_AND_SET}${
                gCfg.LineBreak
            }` +
        ...
        let sheetContent = FMT.replace('{data}', data)
            .replace(nameReg, sheetName)
            .replace('{classDef}', classDef);
        const fileList = new Array<string>();
        for (const st of dataTable.sheetTable) {
            fileList.push(path.basename(st.fileName));
        }
        sheetContent =
            `${gCfg.LineBreak}// excel: ${fileList.join(', ')}${gCfg.LineBreak}` +
    ...
    private GenBaseTypeName(typeDef: TypeDef): string {
        switch (typeDef.__inner__o_type__abcxyz__) {
            ...
            case 'column':
                return (
                    PBNTypeTranslateMap.get(typeDef.__inner__name__abcxyz__ ?? '')?.s ?? 'string'
                );
        }
    ...
    private GenObjectMessageAndGetTypeName(typeDef: ObjectTypeDef): string {
        ...
        let comment = '';
        if (
            typeDef.__inner__def_comment !== undefined &&
            typeDef.__inner__def_comment.self.length > 0
        ) {
        ...
        for (const key in kv) {
            ...
            if (val.__inner__o_type__abcxyz__ === 'array') {
                ...
                if (val.__inner__type__abcxyz__?.__inner__o_type__abcxyz__ === 'array') {
                    typeName = `${ARRAY_TYPE(
                        this.GenArrayMessageAndGetTypeName(val.__inner__type__abcxyz__)
                    )}`;
                } else {
            ...
            if (
                typeDef.__inner__def_comment !== undefined &&
                typeDef.__inner__def_comment.children !== undefined &&
                typeDef.__inner__def_comment.children[key] !== undefined
            ) {
                comment =
                    `    ` +
                    typeDef.__inner__def_comment.children[key].join(`${gCfg.LineBreak}    `);
                if (EnableRegion) {
                    comment = comment
            ...
            if (
                !isBaseType &&
                this._exportCfg.NullableReferenceTypes &&
                !(<ObjectTypeDef>val).__inner__is_struct_parse_mode__
            ) {
                body +=
                    gCfg.LineBreak +
                    comment +
                    ...
                    `#nullable enable${gCfg.LineBreak}` +
                    `    public ${typeName}? ${key} ${GET_AND_SET}` +
                    gCfg.LineBreak +
                    `#nullable disable${gCfg.LineBreak}`;
            } else {
                body +=
        ...
        const classType = typeDef.__inner__is_struct_parse_mode__ ? 'struct' : 'class';
        const classMessage =
            gCfg.LineBreak +
            comment +
            // `[DataContract]${gCfg.LineBreak}` +
            ...
            `[ProtoContract]${gCfg.LineBreak}` +
            `public partial ${classType} ${typeName}` +
            gCfg.LineBreak +
            `{${gCfg.LineBreak}` +
            body +
            `}` +
...
Array(Enum({
	...
	Equip: 2,
        ...
        const flagMode = typeDef.__inner__is_flag__mode ? ', Flags' : '';
    ...
    private async writeFileAsync(file: string, context: string): Promise<void> {
        this._saveFileSet.add(path.basename(file));
        await fs.promises.writeFile(file, this.ARR_FILE_HEADER + context, {
            encoding: 'utf8',
            flag: 'w',
        });
    }
    ...
    // 清理所有的文件
    private async CleanupOldFiles(outdir: string): Promise<void> {
        ...
        for (const fileName of pa) {
            ...
            // ignore ext name
            if (path.extname(fileName) !== this._exportCfg.ExtName) continue;
            const info = fs.statSync(filePath);
            // ignore directory
            if (!info.isFile()) continue;
            // ignore save file
            if (this._saveFileSet.has(path.basename(fileName))) continue;
            // ignore file not auto generate
            const filectx = await fs.promises.readFile(filePath, { encoding: 'utf-8' });
            if (!filectx.startsWith(this.ARR_FILE_HEADER)) continue;
            // delete file and log
...
module.exports = function (exportCfg: utils.ExportCfg): utils.IExportWrapper {
    return new ProtobufNetExport(exportCfg);
};
