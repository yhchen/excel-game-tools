...
////////////////////////////////////////////////////////////////////////////////
class PBExport2 extends utils.IExportWrapper {
	...
	protected async ExportTo(dt: utils.SheetDataTable): Promise<boolean> {
		...
		throw new Error(`proto2 export to folder was not support!`);
	}
	...
	protected async ExportGlobal(): Promise<boolean> {
		...
		if (!this._exportCfg.OutputDataDir) {
			utils.exception("proto2 Export.OutputDataDir was not set!");
		}
		...
		for (let dt of array) {
			...
			let ctx = this.GenSheetType(name, dt.arrTypeHeader, LF);
			if (ctx) {
				data += `${ctx.pbtype}${LF}${LF}`;
			}
		}

		const PackageContent = this._exportCfg.Namespace ? `package ${this._exportCfg.Namespace};${LF}` : "";
		data = `${this._leadTypeList.join(LF)}${data}${LF}`;
		data = `syntax = "proto2";${LF}${LF}${PackageContent}${LF}` + FMT.replace('{data}', data);
	...
	private GenSheetType(sheetName: string, arrHeader: utils.SheetHeader[], LF: string): { pbtype: string; } | undefined {
		...
		for (let header of arrExportHeader) {
			...
			if (defType != undefined) {
				switch (defType.__inner__o_type__abcxyz__) {
					case 'array':
						...
						break;
					case 'object':
						typeName = 'optional ' + this.GenObjectMessageAndGetTypeName(defType, LF);
						break;
					case 'base':
					case 'enum':
					case 'column':
						typeName = 'optional ' + this.GenBaseTypeName(defType);
						break;
					default:
						utils.exception(`call "${utils.yellow_ul('GenTypeName')}" failure`);
				}
			...
			rowType += `        ${typeName} ${this.TranslateColName(header.name)} = ${header.cIdx + 1};${LF}`;
		}
		rowType += `    }${LF}`;
	...
	private GenArrayMessageAndGetTypeName(typeDef: ArrayTypeDef, LF: string): string {
		...
		let message = `message ${arrTypeName} {${LF}    repeated ${typeName} a = 1;${LF}}${LF}`;
		...
		for (const key in kv) {
			...
			if (val.__inner__o_type__abcxyz__ == 'array') {
				if ((val.__inner__type__abcxyz__)?.__inner__o_type__abcxyz__ == 'array') {
					typeName = `repeated ${this.GenArrayMessageAndGetTypeName(val.__inner__type__abcxyz__, gCfg.LineBreak)}`;
				} else {
					typeName = `repeated ${this.GenBaseTypeName(val.__inner__type__abcxyz__)}`;
				}
			} else if (val.__inner__o_type__abcxyz__ == 'object') {
				typeName = `optional ${this.GenObjectMessageAndGetTypeName(val, LF)}`;
			} else {
				typeName = 'optional ' + this.GenBaseTypeName(val);
			}
			body += `    ${typeName} ${key} = ${index++};${LF}`;
		}
		let message = `message ${typeName} {${LF}${body}}${LF}`;
		this._leadTypeList.push(message);
...
Array(Enum({
	...
	private TranslateValue(value: any, type: TypeDef): any {
		if (isArray(value) && type.__inner__o_type__abcxyz__ == 'array' && type.__inner__type__abcxyz__.__inner__o_type__abcxyz__ == 'array') {
			if (value.length <= 0 || !isArray(value[0])) {
				return { a: value };
			}
			const ret = new Array<{ a: any[]; }>();
			for (const subvalue of value) {
				ret.push({
					a: this.TranslateValue(subvalue, type.__inner__type__abcxyz__),
				});
			}
			return ret;
		}
	...
	private async ExportData(dt: utils.SheetDataTable, outputFile: string) {
		...
		const arrTypeHeader = utils.ExecGroupFilter(dt.sheetName, this._exportCfg.GroupFilter, dt.arrTypeHeader);
		if (arrTypeHeader == undefined || arrTypeHeader.length <= 0) {
			console.info(`Pass Sheet ${utils.yellow_ul(dt.sheetName)}: ignore export data ${outputFile} because no column to export.`);
			return;
		}
		const protoEncoder = this._protoRoot.lookupType(`${this._exportCfg.Namespace ? this._exportCfg.Namespace + '.' : ''}${this.TABLE_PREFIX}${dt.sheetName}`);
		...
		for (let row = 0; row < dt.arrValues.length; ++row) {
			...
			for (const hdr of arrExportHeader) {
				...
				newData[this.TranslateColName(hdr.name)] = this.TranslateValue(data[hdr.cIdx], hdr.parser.type);
			}
...
module.exports = function (exportCfg: utils.ExportCfg): utils.IExportWrapper { return new PBExport2(exportCfg); };
