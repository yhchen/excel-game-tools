...
export async function HandleExcelFile(fileName: string): Promise<boolean> {
    try {
        ...
        if (excel.Sheets === undefined) {
            return true;
        }
        for (const sheetName of excel.SheetNames) {
            utils.debug(
                `- Handle excel "${utils.brightWhite(fileName)}" Sheet "${utils.yellow_ul(
                    sheetName
                )}"`
            );
...
export async function HandleCsvFile(fileName: string): Promise<boolean> {
    try {
        ...
        utils.debug(
            `- Handle csv "${utils.brightWhite(fileName)}" Sheet "${utils.yellow_ul(
                csv.sheetName
            )}"`
        );
...
function HandleDataTable(worksheet: IDataLoader, fileName: string): void {
    const res = HandleDataTableStep1(worksheet);
    if (!res) return;
    const dataTable = res.DataTable;
    ...
    if (!oldDataTable) {
        ...
        ExcelWorkSheetHandleListStep.push({
            worksheet: worksheet,
            rIdx: res.rIdx,
            RowMax: res.RowMax,
            ColumnMax: res.ColumnMax,
            DataTable: dataTable,
        });
        return;
    }
    ...
    if (dataTable.arrTypeHeader.length !== oldDataTable.arrTypeHeader.length) {
        utils.exception(
            `found duplicate file name with different header : ${utils.yellow_ul(
                worksheet.sheetName
            )} \n` +
    ...
    for (let i = 0; i < dataTable.arrTypeHeader.length; ++i) {
        if (!utils.IsSheetHeaderSame(dataTable.arrTypeHeader[i], oldDataTable.arrTypeHeader[i])) {
            utils.exception(
                `found duplicate file name with different header [${
                    dataTable.arrTypeHeader[i].name
                }] and [${oldDataTable.arrTypeHeader[i].name}] : ${utils.yellow_ul(
                    worksheet.sheetName
                )} \n` +
                    `at ${utils.yellow_ul(fileName)} \n` +
                    `and ${utils.yellow_ul(oldDataTable.sheetTable[0].fileName)}`
...
function HandleDataTableStep1(worksheet: IDataLoader): Step1Result | undefined {
    if (
        !utils.StrNotEmpty(worksheet.sheetName) ||
        worksheet.sheetName[0] === '!' ||
        worksheet.sheetName[0] === '#'
    ) {
...
function HandleWorksheetNameRow(
    worksheet: IDataLoader,
    rIdx: number,
    RowMax: number,
    ColumnMax: number,
    ...
    for (; rIdx <= RowMax; ++rIdx) {
        ...
        for (let cIdx = 1; cIdx <= ColumnMax; ++cIdx) {
            ...
            if (!/^[a-zA-Z0-9_]+$/.test(name)) {
                utils.exception(
                    `Excel "${utils.yellow_ul(worksheet.fileName)}" ` +
                        `Sheet "${utils.yellow_ul(worksheet.sheetName)} ` +
                        `Cell "${utils.yellow_ul(
                            utils.FMT26.NumToS26(cIdx) + (rIdx + 1).toString()
                        )}" ` +
        ...
        DataTable.arrValues.push({
            worksheet,
            type: utils.ESheetRowType.header,
            values: tmpArry,
            rIdx,
        });
        ++rIdx;
...
function HandleWorksheetGroupFilterRow(
    worksheet: IDataLoader,
    rIdx: number,
    RowMax: number,
    ColumnMax: number,
    ...
    for (; rIdx <= RowMax; ++rIdx) {
        ...
        for (const header of DataTable.arrTypeHeader) {
            ...
            // Check if the group map has the specified key
            if (!Object.prototype.hasOwnProperty.call(gCfg.GroupMap, colGrp)) {
                utils.exception(
                    `Excel "${utils.yellow_ul(worksheet.fileName)}" Sheet "${utils.yellow_ul(
                        worksheet.sheetName
                    )}" ` +
...
export function HandleDataTableStep2(): boolean {
    ExcelWorkSheetHandleListStep.sort((a, b) =>
        a.worksheet.fileName < b.worksheet.fileName ? -1 : 1
    );
    for (let i = ExcelWorkSheetHandleListStep.length - 1; i > -1; --i) {
        const s = ExcelWorkSheetHandleListStep[i];
        // handle type row
        try {
            s.rIdx = HandleWorksheetTypeRowPaste(
                s.worksheet,
                s.rIdx,
                s.RowMax,
                s.ColumnMax,
            ...
            s.rIdx = HandleWorksheetCollectBaseDataRow(
                s.worksheet,
                s.rIdx,
                s.RowMax,
                s.ColumnMax,
...
export function HandleDataTableStep3(): boolean {
    ...
    for (const [, DataTable] of utils.ExportExcelDataMap) {
        for (let colIdx = 0; colIdx < DataTable.arrTypeHeader.length; ++colIdx) {
            const header = DataTable.arrTypeHeader[colIdx];
            // already deal
            ...
            for (let rowIdx = 0; rowIdx < DataTable.arrValues.length; ++rowIdx) {
                ...
                try {
                    ...
                    foundError = true;
                    utils.exceptionRecord(
                        `Excel "${utils.yellow_ul(row.worksheet.fileName)}" ` +
                            `Sheet Row "${utils.yellow_ul(
                                DataTable.sheetName + '.' + utils.yellow_ul(header.name)
                            )}" High Type format error ` +
                            `Cell "${utils.yellow_ul(
                                utils.FMT26.NumToS26(cIdx) + (row.rIdx + 1).toString()
                            )}" ` +
                            ` "${utils.yellow_ul(data?.w ?? '<no data>')}"!`,
                        ex
                    );
                }
            }
    ...
    utils.logger(
        `${foundError ? utils.red('[FAILURE]') : utils.green('[SUCCESS]')} ` +
            `CHECK ALL TypeChecker Parser DONE. Total Use Tick : ${utils.green(
                utils.TimeUsed.LastElapse()
            )}`
    );
    return !foundError;
}

function HandleWorksheetTypeRow(
    worksheet: IDataLoader,
    rIdx: number,
    RowMax: number,
    ColumnMax: number,
    ...
    for (; rIdx <= RowMax; ++rIdx) {
        ...
        if (
            firstCell === undefined ||
            !utils.StrNotEmpty(firstCell.w) ||
            zeroCell === undefined ||
            !utils.StrNotEmpty(zeroCell.w)
        ) {
            continue;
        }
        if (firstCell.w[0] === '#') {
        ...
        for (const col of arrHeaderName) {
            ...
            if (cell === undefined || cell.w === undefined) {
                utils.exception(
                    `Excel "${utils.yellow_ul(worksheet.fileName)}" ` +
                        `Sheet "${utils.yellow_ul(
                            worksheet.sheetName
                        )}"  Type Row "${utils.yellow_ul(col.name)}" not found!`
                );
            }
            try {
                ...
                utils.exception(
                    `Excel "${utils.yellow_ul(worksheet.fileName)}" Sheet "${utils.yellow_ul(
                        ...
                        ` "${utils.yellow_ul(col.name)}" format error "${utils.yellow_ul(
                            cell.w
                        )}"!`,
                    ex
                );
...
function HandleWorksheetTypeRowPaste(
    worksheet: IDataLoader,
    rIdx: number,
    RowMax: number,
    ColumnMax: number,
        ...
        try {
            ...
            utils.exception(
                `Excel "${utils.yellow_ul(worksheet.fileName)}" Sheet "${utils.yellow_ul(
                    ...
                    ` "${utils.yellow_ul(col.name)}" format error "${utils.yellow_ul(col.stype)}"!`,
                ex
            );
        }
    }
...
function HandleWorksheetCollectBaseDataRow(
    worksheet: IDataLoader,
    rIdx: number,
    RowMax: number,
    ColumnMax: number,
    ...
    for (; rIdx <= RowMax; ++rIdx) {
        ...
        TypeDefParser.setHeaderNameMap(DataTable.sheetName, DataTable.arrHeaderNameMap);
        for (const header of arrHeaderName) {
            if (!header.parser.canCollectionOnBasePass) {
                ...
                try {
                    ...
                    tmpArry[header.cIdx] = header.parser.ParseContent(cell);
                } catch (ex) {
                    ...
                    utils.exceptionRecord(
                        `Excel "${utils.yellow_ul(worksheet.fileName)}" Sheet "${utils.yellow_ul(
                            ...
                            `Cell "${utils.yellow_ul(
                                utils.FMT26.NumToS26(header.cIdx) + (rIdx + 1).toString()
                            )}" ` +
                            `Parse Data "${utils.yellow_ul(value)}" With ${utils.yellow_ul(
                                header.parser.s
                            )} ` +
                            `Cause utils.exception!`,
                        ex
                    );
                    // hasException = true;
                }
