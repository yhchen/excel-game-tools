...
function printHelp(errMsg?: string): never {
	...
	utils.logger(`--debug-output : 1 : enabled output more debug information. 0 : turn off debug information.`);
	process.exit(-1);
}
	...
	try {
		...
		for (let i = 2; i < process.argv.length; ++i) {
			...
			if (cmd == '-h' || cmd == '--h' || cmd == '/h' || cmd == '/?') {
				...
				++i;
				if (i >= process.argv.length) printHelp(`--debug-output must set a value. [1] or [0]`);
